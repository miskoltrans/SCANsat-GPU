...
namespace SCANsat
{
	...
	public static class SCANUtil
	{
		...
		/// <summary>
		/// Read the body's authored height range from a SCANSAT_TERRAIN node, if a pack or patch ships
		/// one. Same shape as SCANSAT_BODY_TEXTURES: a top-level node keyed by body name.
		///
		///   SCANSAT_TERRAIN:NEEDS[RealSolarSystem]
		///   {
		///       name = Mimas
		///       minHeightRange = -400
		///       maxHeightRange = 5100
		///   }
		/// </summary>
		private static bool authoredTerrainRange(CelestialBody b, ref float min, ref float max)
		{
			ConfigNode[] nodes = GameDatabase.Instance.GetConfigNodes("SCANSAT_TERRAIN");
			...
			for (int i = 0; i < nodes.Length; i++)
			{
				...
				if (!node.HasValue("name") || node.GetValue("name") != b.bodyName)
				{
					continue;
				}

				float authoredMin, authoredMax;

				if (!float.TryParse(node.GetValue("minHeightRange"), out authoredMin)
					|| !float.TryParse(node.GetValue("maxHeightRange"), out authoredMax)
					|| authoredMax <= authoredMin)
				{
					SCANlog("[{0}] SCANSAT_TERRAIN node is unusable (needs minHeightRange < maxHeightRange); ignoring it", b.name);
					return false;
				}
		...
		public static SCANterrainConfig generateTerrainConfig(CelestialBody b)
		{
			...
			if (authoredTerrainRange(b, ref newMin, ref newMax))
			{
				...
				SCANterrainConfig authored = new SCANterrainConfig(newMin, newMax, b.ocean ? (float?)0 : null, PaletteLoader(SCANconfigLoader.SCANNode.DefaultPalette, 7), 7, false, false, b);
				authored.AutoGenerated = false;   // measured and declared, never overridden by a guess
				SCANcontroller.addToTerrainConfigData(b.bodyName, authored);
				return authored;
			}
			...
			config.AutoGenerated = true;   // guessed from PQS, not authored in a cfg and not set by the user
